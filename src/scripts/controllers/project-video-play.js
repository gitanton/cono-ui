...
angular.module('conojoApp')
    .controller('ProjectVideoPlayCtrl', function ($rootScope, $scope, $http, $location, $routeParams, ENV, ModalService, videoService, NAV) {
        ...
        $scope.init = function () {
            $http({
                ...
                return videoService.getForProject($scope.activeProjectUuid).then(function (videos) {
                    if (videos.length < 1) {
                        $location.path('/' + NAV.PROJECT_VIDEO_UPLOAD + '/' + $scope.activeProjectUuid);
                    } else {
